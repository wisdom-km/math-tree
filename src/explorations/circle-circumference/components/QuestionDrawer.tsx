...
/** 老师提问卡侧抽屉（半宽，不挡主舞台关键控件）。步骤 8 显示全部；其余按当前步骤过滤。 */
    ...
    <aside className="qdrawer" aria-label="老师提问卡">
      ...
      {flat.map((item, i) => (
        <div key={item.key} className={`q ${s.askedQuestions.includes(item.key) ? "asked" : ""} ${i === cursor ? "" : "muted"}`}>
          <button
            className="btn"
            aria-pressed={s.askedQuestions.includes(item.key)}
            aria-label="标记已问"
            onClick={() => dispatch({ type: "MARK_ASKED", key: item.key })}
          >
      ...
      {handVote && (
        <HandVoteChart state={s} dispatch={dispatch} step={s.step} options={handVote} title="投屏举手" />
      )}
      {quickVote && current && (
        <HandVoteChart state={s} dispatch={dispatch} step={8} options={QUICK_VOTE} title={`临时举手：${current.q}`} />
      )}
      <div className="row">
        {flat.length > 1 && (
